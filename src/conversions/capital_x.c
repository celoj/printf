...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/05/30 23:19:48 by dcelojev          #+#    #+#             */
/*   Updated: 2019/07/09 13:51:49 by dcelojev         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char		*perform_function_for_flag(char flag, void *data)
{
	...
	if (flag == 'h' || flag == 'l')
	{
		hex = ft_ulltoa_base((unsigned long long)data, 16, 1);
	}
	else if (flag == 'j')
	{
		hex = ft_uimttoa_base((intmax_t)data, 16, 1);
	}
	else if (flag == 'z')
	{
		hex = ft_sttoa_base((size_t)data, 16, 1);
	}
	else if (flag == 't')
	{
		hex = ft_pdttoa_base((ptrdiff_t)data, 16, 1);
	}
	return (hex);
}
		...
		int print_prefix)
{
	...
	if (flags->width_specifier > 0)
	{
		...
		(flags->precision > data_length ? flags->precision : data_length)
		- (print_prefix * 2));
		else
	...
	if (is_flag_enabled('.', flags) && flags->precision > 0)
		chars_printed += enforce_width_specifier('0', flags->precision
				- data_length);
...
int				capital_x(t_flags *flags, void *data)
{
	...
	return (handle_global_flags(flags, ft_strlen(hex), (int)data != 0
			&& can_print_hex_prefix(flags)) + ft_putstr_ret(hex));
}
